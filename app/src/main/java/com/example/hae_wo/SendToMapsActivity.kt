...
class SendToMapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        if (length > 0) {
            ...
            for (i in 0..lengthNew - 2 step 2) {
                Log.e("i:", i.toString())
                mMap.addCircle(
                    CircleOptions().center(latlngArray[i]).radius(1.0).strokeColor(Color.BLUE)
                        .fillColor(
                            Color.BLUE
                        )
                )
                mMap.addCircle(
                    CircleOptions().center(latlngArray[i + 1]).radius(1.0).strokeColor(Color.BLUE)
                        .fillColor(
                            Color.BLUE
                        )
                )
                mMap.addPolyline(
                    PolylineOptions().add(latlngArray[i]).add(latlngArray[i + 1]).color(
                        Color.RED
                    )
                )
            }
